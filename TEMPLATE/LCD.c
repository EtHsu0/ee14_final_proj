...
void LCD_PIN_Init(void){
	...
	GPIOA->MODER &= ~(0xC03FF000); // 0b1100 0000 0011 1111 1111 0000 0000 0000
	GPIOA->MODER |= 0x802AA000; // 0b1000 

	// Configure Port A Pins 6, 7, 8, 9, 10, 15 as Alternate Function 11
	GPIOA->AFR[0] &= ~(0xFF000000); // 0x1111 1111 0000 0000 0000
	GPIOA->AFR[0] |= 0xBB000000; // 0x1011 1011 0000 0000 0000
	GPIOA->AFR[1] &= ~(0xF0000FFF);
	GPIOA->AFR[1] |= 0xB0000BBB;

	// Port B Pins 0, 1, 4, 5, 9, 12, 13, 14, 15
	GPIOB->MODER &= ~(0xFF0C0F0F); // 0b1111 1111 0000 1100 0000 1111 0000 1111
	GPIOB->MODER |= 0xAA080A0A ; // 0b1010 1010 0000 1000 0000 1010 0000 1010
	
	GPIOB->AFR[0] &= ~(0x00FF00FF); // 0b0000 0000 1111 1111 0000 0000 1111 1111
	GPIOB->AFR[0] |= 0x00BB00BB;
	GPIOB->AFR[1] &= ~(0xFFFF00F0);
	GPIOB->AFR[1] |= 0xBBBB00B0;

	// Port C Pins 8, 7, 6, 5, 4, 3 
	GPIOC->MODER &= ~(0x0003FFC0); // 0b0000 0000 0000 0011 1111 1111 1100 0000
	GPIOC->MODER |= 0x0002AA80; // 0b0000 0000 0000 0010 1010 1010 1000 0000
	...
	GPIOD->MODER &= ~(0xFFFF0000); // 0b1111 1111 1111 1111 0000 0000 0000 0000
	GPIOD->MODER |= 0xAAAA0000;
	
	// Don't need mask/value for afr[0]?
